"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Packer = void 0;
var readLine_1 = require("../src/utils/readLine");
var error_1 = __importDefault(require("./error"));
var Packer = /** @class */ (function () {
    function Packer() {
    }
    Packer.pack = function (inputFile) {
        return __awaiter(this, void 0, void 0, function () {
            var fileData_1, result_1, parseOutput, getPackagesSum_1, getSelectedPackages_1, selectValidPackagesByWeightAndPrice, error_2;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, (0, readLine_1.processLineByLine)(inputFile)];
                    case 1:
                        fileData_1 = _a.sent();
                        result_1 = [];
                        if (!fileData_1 || JSON.stringify(fileData_1) === JSON.stringify({})) {
                            throw new error_1.default(400, 'invalid data');
                        }
                        parseOutput = function (output) {
                            var trimOutput = output.filter(function (item) { return item; });
                            return trimOutput
                                .join('\r\n')
                                .split(' ')
                                .join('');
                        };
                        getPackagesSum_1 = function (items, maxWeight) {
                            var len = items.length;
                            var sets = {};
                            for (var i = 0; i < len - 1; i++) {
                                for (var j = i + 1; j < len; j++) {
                                    if (items[i].weight + items[j].weight <= maxWeight) {
                                        var indexes = items[i].index + ", " + items[j].index;
                                        var values = {
                                            indexes: indexes,
                                            sumWeight: items[i].weight + items[j].weight,
                                            sumPrice: items[i].price + items[j].price
                                        };
                                        sets[indexes] = values;
                                    }
                                    var value = {
                                        indexes: items[i].index,
                                        sumWeight: items[i].weight,
                                        sumPrice: items[i].price
                                    };
                                    sets[items[i].index] = value;
                                }
                            }
                            return sets;
                        };
                        getSelectedPackages_1 = function (items) {
                            var values = Object.values(items);
                            // get package with highest price
                            var max = values.reduce(function (prev, current) {
                                return prev.sumPrice > current.sumPrice ? prev : current;
                            }, 0);
                            return max;
                        };
                        selectValidPackagesByWeightAndPrice = function () {
                            var packageItems = [];
                            // remove any weight of items greater than the maxWeight or item price is greater than 100
                            packageItems = fileData_1.map(function (packages) {
                                var maxWeight = packages.maxWeight, items = packages.items;
                                return {
                                    maxWeight: maxWeight,
                                    items: items.filter(function (item) { return item.weight < maxWeight && item.price < 100; })
                                };
                            });
                            // Do logic for packages with single and multiple items resp.
                            packageItems = packageItems.map(function (_a) {
                                var maxWeight = _a.maxWeight, items = _a.items;
                                // returns "_" when maxWeight is greater than 100
                                if (maxWeight > 100) {
                                    return result_1.push('_');
                                }
                                // returns "_" when no items in the package
                                if (!items.length) {
                                    return result_1.push('_');
                                }
                                // returns the first index for packages with only one item
                                if (items.length === 1) {
                                    result_1.push(items[0].index);
                                }
                                // items with multiple packages
                                if (items.length >= 1) {
                                    var packagesSum = getPackagesSum_1(items, maxWeight);
                                    var indexes = getSelectedPackages_1(packagesSum).indexes;
                                    result_1.push(indexes);
                                }
                            });
                        };
                        // calls the function to get appropriate packages
                        selectValidPackagesByWeightAndPrice();
                        return [2 /*return*/, parseOutput(result_1)];
                    case 2:
                        error_2 = _a.sent();
                        return [2 /*return*/, {
                                statusCode: error_2.statusCode,
                                message: error_2.message
                            }];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    return Packer;
}());
exports.Packer = Packer;
